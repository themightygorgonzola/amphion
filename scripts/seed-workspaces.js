...
const WORKSPACES = [
...

  {
    ...
    ppmService:  null,
    conventions: [
      'Monorepo under packages/: controller (port 7000), dashboard (port 3000), agent (port 7002)',
...

  {
    ...
    description: 'AI broker/brain: 4-stage pipeline (context→dispatch→orchestrate→voice), RAG/pgvector, MCP agents, Electron tray UI',
    language:    'Node.js',
    buildCmd:    'node scripts/start.js',
    ppmService:  null,
    conventions: [
      'Supervisor in scripts/start.js — auto-started via Windows Task Scheduler',
      'Broker HTTP API on port 3000 (POST /query for SSE chat)',
      'MCP server at amphion/mcp-server/index.js (stdio, 5+1 tools)',
      'Ingest pipeline: inbox → review → approved → ingested (only .md/.txt/.html/.htm allowed)',
      'pgvector for RAG (Docker), SQLite for conversations/context (native Node)',
      'ReAct loop cap: 16 steps (bumped from 8)',
    ],
    keyDirs: [
      { path: 'apps/broker/src/',  description: 'Core broker: context-assembler, dispatcher, orchestrator, voice-layer, db' },
      { path: 'agents/research/',  description: 'Research agent: find_path, browse_path, search_local (ripgrep), search_hybrid' },
      { path: 'agents/dev/',       description: 'Dev agent — EMPTY (.gitkeep), to be built' },
      { path: 'mcp-server/',       description: 'Standalone MCP stdio server — 6 tools for IDE integration' },
      { path: 'data/inbox/',       description: 'Ingest inbox — drop files here for quarantine review' },
      { path: 'data/memory.db',    description: 'SQLite: conversations, workspace_registry, query_log' },
      { path: 'scripts/',          description: 'Admin scripts: migrate.js, seed-workspaces.js, start.js' },
    ],
